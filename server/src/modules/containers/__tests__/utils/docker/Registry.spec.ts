...
import { Kind } from '../../../../../application/services/components/application/services/components/core/Component';
import AbstractRegistry from '../../../../../application/services/components/registries/Registry';
...
describe('testing Registry', () => {
  ...
  test('getImageManifestDigest should throw when no digest found', async () => {
    ...
    await expect(
      ...
      new Error(
        'getImageManifestDigest - Unexpected error; no manifest found for image for image "image"',
      ),
    );
  });
