 ...
 * Please use the new WatcherEngineService in application/services/engine/watcher-engine.service.ts
 */
...
import { WatcherEngineServiceInterface, WATCHER_ENGINE_SERVICE } from '../application/interfaces/watcher-engine-service.interface';
...
export class WatcherEngineService implements OnModuleInit, OnModuleDestroy {
  private readonly stdLogger = new Logger(WatcherEngineService.name);
  ...
  constructor(
    ...
    private readonly newWatcherEngineService: WatcherEngineServiceInterface,
  ) {
    // Initialize the registriesGetter in utils.ts
    setRegistriesGetter(() => this.getRegistries());
    this.stdLogger.warn('This service is deprecated and will be removed in a future version');
  }
   ...
   * @deprecated Use the new WatcherEngineService
   */
  findRegisteredDockerComponent(watcher: string): Docker | undefined {
    this.stdLogger.warn('Method findRegisteredDockerComponent is deprecated. Please use the new WatcherEngineService.');
    try {
      return this.newWatcherEngineService.findRegisteredDockerComponent(watcher);
    } catch (error) {
