...
import { DEVICE_DOWNTIME_EVENT, DeviceDownTimeEventDocument } from '../schemas/device-downtime-event.schema';
...
export class DeviceDownTimeEventRepository implements IDeviceDownTimeEventRepository {
  constructor(
    @InjectModel(DEVICE_DOWNTIME_EVENT) private downtimeEventModel: Model<DeviceDownTimeEventDocument>,
  ...
  async closeDownTimeEvent(deviceId: string): Promise<void> {
    ...
    const downtimeEvent = await this.downtimeEventModel.findOne({
      deviceId,
    ...
    await this.downtimeEventModel.updateOne(
  ...
  async sumTotalDownTimePerDeviceOnPeriod(from: Date, to: Date): Promise<{ deviceId: string; duration: number }[]> {
    logger.info(`Calculating total downtime per device from ${from} to ${to}`);

    const result = await this.downtimeEventModel.aggregate([
      {
        $match: {
