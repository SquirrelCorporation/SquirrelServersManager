...
import { BadRequestException, EntityNotFoundException } from '../../../../infrastructure/exceptions';
...
describe('DashboardController', () => {
  ...
  describe('getSingleAveragedStatsByDevicesAndType', () => {
    it('should return averaged statistics for valid request', async () => {
      ...
      // Test
      const result = await dashboardController.getSingleAveragedStatsByDevicesAndType(type, query, body);
      
      // Verify
      expect(mockDevicesService.findByUuids).toHaveBeenCalledWith(body.devices);
      
      // Verify the date conversion
      const fromDate = DateTime.fromJSDate(new Date(query.from.split('T')[0])).endOf('day').toJSDate();
      const toDate = DateTime.fromJSDate(new Date(query.to.split('T')[0])).endOf('day').toJSDate();
    ...
    it('should throw BadRequestException when date range is not provided', async () => {
      ...
      // Test & Verify
      await expect(
        dashboardController.getSingleAveragedStatsByDevicesAndType(type, query as DashboardStatQueryDto, body)
      ).rejects.toThrow(BadRequestException);
  ...
  describe('getStatsByDevicesAndType', () => {
    ...
    it('should throw BadRequestException when devices are not provided', async () => {
      ...
      // Test & Verify
      await expect(
        dashboardController.getStatsByDevicesAndType(type, body, query)
      ).rejects.toThrow(BadRequestException);
