...
describe('MetricsService', () => {
  ...
  describe('setMetric', () => {
    ...
    it('should throw error when metric type is not found', async () => {
      ...
      await expect(service.setMetric(invalidType, 50, 'device-123'))
        .rejects.toThrow(`Metric type ${invalidType} not found`);
    });
    
    it('should throw error when device UUID is not provided', async () => {
      await expect(service.setMetric(MetricType.CPU_USAGE, 50, ''))
        .rejects.toThrow('Device UUID is required');
    });
    
    it('should throw error when value is not a number', async () => {
      await expect(service.setMetric(MetricType.CPU_USAGE, NaN, 'device-123'))
        .rejects.toThrow(`Invalid value for metric ${MetricType.CPU_USAGE}: NaN`);
    });
  });
  ...
  describe('setContainerMetric', () => {
    ...
    it('should throw error when metric type is not found', async () => {
      ...
      await expect(service.setContainerMetric(invalidType, 50, 'container-123'))
        .rejects.toThrow(`Metric type ${invalidType} not found`);
    });
    
    it('should throw error when container ID is not provided', async () => {
      await expect(service.setContainerMetric(MetricType.CONTAINER_CPU_USAGE, 50, ''))
        .rejects.toThrow('Container ID is required');
    });
    
    it('should throw error when value is not a number', async () => {
      await expect(service.setContainerMetric(MetricType.CONTAINER_CPU_USAGE, NaN, 'container-123'))
  ...
  describe('setMetrics', () => {
    ...
    it('should throw error when one of the metrics fails', async () => {
      ...
      await expect(service.setMetrics(metrics, deviceUuid))
        .rejects.toThrow();
    });
  });
