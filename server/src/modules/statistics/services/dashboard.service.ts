...
export class DashboardService {
  ...
  async getSystemPerformance() {
    ...
    const currentMemResult = await this.prometheusService.queryAveragedStatByType('memFree', {
      days: 7,
      offset: 0,
    });
    const previousMemResult = await this.prometheusService.queryAveragedStatByType('memFree', {
      days: 14,
      offset: 7,
    });
    const currentCpuResult = await this.prometheusService.queryAveragedStatByType('cpu', {
      days: 7,
      offset: 0,
    });
    const previousCpuResult = await this.prometheusService.queryAveragedStatByType('cpu', {
      days: 14,
      offset: 7,
    });
