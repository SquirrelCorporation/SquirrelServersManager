...
import { PI_MODEL_NO } from '../constants/raspberry.constants';
import {
  Callback,
  RemoteExecutorType,
  RemoteExecutorTypeWithCallback,
} from '../../types/remote-executor.types';
import { execOptsLinux } from '../utils/system-utils';
...
export abstract class RemoteOS {
  ...
  protected async detectOS(): Promise<'linux' | 'darwin' | 'windows'> {
    ...
    throw new Error(`Unknown OS or OS detection failed ${err?.message}`);
  }
