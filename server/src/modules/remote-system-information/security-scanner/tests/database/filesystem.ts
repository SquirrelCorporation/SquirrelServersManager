...
const fileSystemTests: TestGroup = {
  ...
  tests: [
    ...
    },
    {
      ...
      evaluate: (output: string) => {
        const files = output.split('\n').filter((line) => line.length > 0);
        const knownSuid = ['/usr/bin/sudo', '/usr/bin/passwd', '/usr/bin/su', '/usr/bin/ping'];
        const suspicious = files.filter((file) => !knownSuid.some((known) => file.includes(known)));
        ...
        return {
          ...
          details:
            suspicious.length > 0
              ? `Suspicious files: ${suspicious.slice(0, 3).join(', ')}`
              : undefined,
    ...
    },
    {
      ...
      evaluate: (output: string) => {
        ...
        return {
          ...
          details: files.length > 0 ? `Example files: ${files.slice(0, 3).join(', ')}` : undefined,
    ...
    },
    {
      ...
      evaluate: (output: string) => {
        ...
        return {
          ...
          recommendation: !hasSticky
            ? 'Set sticky bit on temp directories'
            : !secureMount
              ? 'Mount temp directories with noexec,nosuid options'
              : undefined,
        };
      },
