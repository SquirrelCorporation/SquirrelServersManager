...
const networkTests: TestGroup = {
  ...
  tests: [
    ...
    },
    {
      ...
      evaluate: (output: string) => {
        ...
        return {
          ...
          details: dangerous.length > 0 ? `Open dangerous ports: ${dangerous.join(', ')}` : undefined,
    ...
    },
    {
      ...
      frameworks: ['CIS', 'NIST'],
      platforms: ['linux'],
      command: platform => platform === 'linux' ? 
    ...
    },
    {
      ...
      frameworks: ['CIS'],
      platforms: ['linux', 'darwin'],
      command: 'systemctl status systemd-timesyncd.service || systemctl status ntpd.service || systemctl status chronyd.service',
      evaluate: (output: string) => {
    ...
    },
    {
      ...
      evaluate: (output: string) => {
        ...
        return {
          ...
          message: isIgnored ? 
            'ICMP broadcasts are ignored' : 
            'ICMP broadcasts are accepted',
