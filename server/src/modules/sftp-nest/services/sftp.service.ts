...
import { sendFile } from '../../ssh/socket-utils/file-stream';
import { SftpSessionDto } from '../dto/sftp-session.dto';
import {
  SftpChmodOptions,
  SftpDeleteOptions,
  ...
  SftpSession,
  SftpStatusMessage,
} from '../interfaces/sftp.interfaces';
...
export class SftpService {
  ...
  async download(clientId: string, filePath: string): Promise<void> {
    ...
    try {
      ...
      // Use the existing file-stream utility
      sendFile(session.client, filePath, filename);
    } catch (error: any) {
      this.logger.error(`Error downloading file: ${error.message}`);
      session.client.emit(SsmEvents.SFTP.DOWNLOAD, {
        status: 'ERROR',
        message: error.message,
      });
    }
