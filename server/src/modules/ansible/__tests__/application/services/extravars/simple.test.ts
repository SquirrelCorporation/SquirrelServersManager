...
import { SsmAnsible, API, SsmAgent } from 'ssm-shared-lib';
...
describe('ExtraVarsService', () => {
  ...
  describe('findValueOfExtraVars', () => {
    it('should return substituted extra vars', async () => {
      ...
      const forcedValues: API.ExtraVars = [
        { extraVar: 'test', value: 'test-value' }
      ];
    ...
    it('should throw error for required extraVars with no value', async () => {
      ...
      // Test & Verify
      await expect(
        extraVarsService.findValueOfExtraVars(extraVars)
      ).rejects.toThrow('ExtraVar value not found ! (test)');
    });
    ...
    it('should handle context variables for single device', async () => {
      ...
      // Test
      const result = await extraVarsService.findValueOfExtraVars(extraVars, undefined, false, [deviceId]);
    ...
    it('should handle unknown context variables', async () => {
      ...
      // Test
      const result = await extraVarsService.findValueOfExtraVars(extraVars, undefined, true, [deviceId]);
