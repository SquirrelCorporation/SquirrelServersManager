...
describe('GalaxyService', () => {
  ...
  describe('getAnsibleGalaxyCollection', () => {
    ...
    it('should throw error when namespace or name is missing', async () => {
      await expect(service.getAnsibleGalaxyCollection(undefined, 'test-collection')).rejects.toThrow('Namespace and name are required');
      await expect(service.getAnsibleGalaxyCollection('test-namespace', undefined)).rejects.toThrow('Namespace and name are required');
    });
  });
  ...
  describe('installCollection', () => {
    it('should call ansible command service to install collection', async () => {
      ...
      expect(mockAnsibleCommandService.installAnsibleGalaxyCollection).toHaveBeenCalledWith('test-collection', 'test-namespace');
    });

    it('should handle error during installation', async () => {
      mockAnsibleCommandService.installAnsibleGalaxyCollection.mockRejectedValueOnce(new Error('Installation error'));
      
      await expect(service.installCollection('test-namespace', 'test-collection')).rejects.toThrow('Installation error');
    });
  });
});