...
export class InventoryTransformerService {
   ...
   */
  async inventoryBuilder(devicesAuth: DeviceAuth[], execUuid: string): Promise<Playbooks.Hosts> {
  ...
  async inventoryBuilderForTarget(
    ...
    for (const deviceAuth of devicesAuth) {
      this.logger.log(`Building inventory for ${deviceAuth.device?.uuid}`);
      ansibleInventory[
   ...
   */
  private async getAuth(deviceAuth: Partial<DeviceAuth>, execUuid: string): Promise<Auth> {
    const auth: Auth = {};
