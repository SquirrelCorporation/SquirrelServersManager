...
import { CONTAINER_STACKS_REPOSITORY_ENGINE_SERVICE, IContainerStacksRepositoryEngineService } from '../../../application/interfaces/container-stacks-repository-engine-service.interface';

describe('IContainerStacksRepositoryEngineService Interface', () => {
  it('should have the correct constant value', () => {
    expect(CONTAINER_STACKS_REPOSITORY_ENGINE_SERVICE).toBe('CONTAINER_STACKS_REPOSITORY_ENGINE_SERVICE');
  });
  ...
  it('should be able to mock service method implementations', async () => {
    ...
    expect(await mockService.cloneRepository('https://github.com/test/repo.git', '/path/to/repo')).toBe(true);
    expect(await mockService.pullRepository('/path/to/repo')).toBe(true);
    expect(await mockService.getRepositoryComponents('/path/to/repo')).toEqual(['component1', 'component2']);

    // Verify the methods were called with the correct arguments
    expect(mockService.cloneRepository).toHaveBeenCalledWith('https://github.com/test/repo.git', '/path/to/repo');
