...
import { SsmGit, SsmAlert } from 'ssm-shared-lib';

import { IGitPlaybooksRepositoryComponent, GitComponentOptions } from '../interfaces/playbooks-repository-component.interface';
 ...
 */
@Injectable()
export class GitPlaybooksRepositoryComponent extends PlaybooksRepositoryComponent implements IGitPlaybooksRepositoryComponent {
  ...
  async initializeWithOptions(options: GitComponentOptions): Promise<void> {
    const { uuid, name, branch, email, gitUserName, accessToken, remoteUrl, gitService, ignoreSSLErrors } = options;
  ...
  async syncFromRepository(): Promise<void> {
    try {
      ...
      this.logger.error(`Error syncing from remote repository: ${error?.message || 'Unknown error'}`);
  ...
  async commitAndSync(): Promise<void> {
    try {
      ...
      this.logger.error(`Error committing and syncing to remote repository: ${error?.message || 'Unknown error'}`);
